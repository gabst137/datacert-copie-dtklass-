...
// External data flow to SEE exclusively
const HEADERS = [
  'Destinatar / Recipient',
  'Categorie / Category',
  'Țara destinatară / Destination state',
  'Baza legală a transferului / Legal basis of data transfer',
  'Modalitate transfer / Type of transfer',
];
...
function ExternalSEEFlowTab({ peopleData = {}, onPeopleDataChange }) {
  const newSeeRef = useRef(null);
  // Store matrix under peopleData.externalSEE
  const data = peopleData.externalSEE || {};
  ...
  // Rows are fully custom for this section (organisation/partner)
  const rows = Object.keys(data).map((k) => ({ id: k, label: data[k]?.__label || 'Organizație/ partener / Organisation/ Partner' }));
  ...
  const addRow = (label) => {
    ...
    const updated = {
      ...data,
      [id]: {
        __label: label || 'Organizație/ partener / Organisation/ Partner',
        recipient: '',
        category: '',
        destState: '',
        legalBasis: '',
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-md p-4">
      <div className="text-sm font-medium" style={{ marginBottom: 12 }}>
        Fluxul extern al datelor (în afara Operatorului), exclusiv către SEE
      </div>

      <div className="text-xs text-gray-600" style={{ marginBottom: 8 }}>
        Folosește câte o intrare per organizație/partener.
      </div>

      <div>
        {rows.map((row) => {
          const rowData = data[row.id] || {};
          const completedCount = ['recipient','category','destState','legalBasis','transferType']
              ...
              {open[row.id] && (
                <div className="bg-gray-50 rounded-lg" style={{ padding: 14, border: '1px solid #e5e7eb', borderTop: '0' }}>
                  <div className="grid" style={{ gap: 12 }}>
                    <FieldRow label={HEADERS[0]} value={rowData.recipient || ''} onChange={(v) => updateCell(row.id, 'recipient', v)} />
                    <FieldRow label={HEADERS[1]} value={rowData.category || ''} onChange={(v) => updateCell(row.id, 'category', v)} />
                    <FieldRow label={HEADERS[2]} value={rowData.destState || ''} onChange={(v) => updateCell(row.id, 'destState', v)} />
                    <FieldRow label={HEADERS[3]} value={rowData.legalBasis || ''} onChange={(v) => updateCell(row.id, 'legalBasis', v)} />
                    <FieldRow label={HEADERS[4]} value={rowData.transferType || ''} onChange={(v) => updateCell(row.id, 'transferType', v)} />
                  </div>
      ...
      <div className="border-t border-gray-200" style={{ marginTop: 16, paddingTop: 12 }}>
        <div className="text-sm font-medium" style={{ marginBottom: 8 }}>Adaugă organizație/partener</div>
        <div className="flex" style={{ gap: 8 }}>
          <input
            id="newSeeOrgLabel"
            ref={newSeeRef}
            type="text"
            placeholder="Organizație/ partener / Organisation/ Partner"
          ...
          <button
            onClick={() => {
              const input = newSeeRef.current;
              const name = sanitizeInput(input?.value || '', 200);
              addRow(name || 'Organizație/ partener / Organisation/ Partner');
...
export default memo(ExternalSEEFlowTab);
