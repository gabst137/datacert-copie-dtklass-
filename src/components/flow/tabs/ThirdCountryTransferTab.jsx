...
function ThirdCountryTransferTab({ legalData = {}, onLegalDataChange }) {
  const newThirdRef = useRef(null);
  // New matrix lives under legalData.thirdCountries
  const data = legalData.thirdCountries || {};

  const update = (updated) => {
    onLegalDataChange?.({
      ...legalData,
      thirdCountries: updated,
    });
  };
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-md p-4">
      <div className="text-sm font-medium" style={{ marginBottom: 12 }}>
        Transfer către state terțe / organizații internaționale
      </div>

      <div className="text-xs text-gray-600" style={{ marginBottom: 8 }}>
        Adaugă câte o intrare per organizație/partener.
      </div>
      ...
      <div className="border-t border-gray-200" style={{ marginTop: 16, paddingTop: 12 }}>
        ...
        <div className="flex" style={{ gap: 8 }}>
          ...
          <button
            onClick={() => {
              const input = newThirdRef.current;
...
export default memo(ThirdCountryTransferTab);
